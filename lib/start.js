...
const start = async function ({ app, host = undefined, port, consul, requestTimeout, headersTimeout }) {
  if (!app) {
  ...
  const networkInterfaces = await create({ app, host, port, consul, requestTimeout, headersTimeout });
